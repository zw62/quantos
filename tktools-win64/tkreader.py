...
def tk_to_h5(user, passwd, tkfile, universe, begin_time, end_time, h5file):    
    try:
        file = tkfile
        reader = TkReader()
        if( not reader.login(user, passwd)):
            return False
        if( not reader.open(file, universe, begin_time, end_time)):
            return False
        print("open finished")
        tk = reader.get_next()
...
if __name__ == '__main__':
    user = "phone number"
    passwd = "token number"
    tkfile = "SHF20171214.tk"
    h5file = "SHF20171214-tk.h5"
    start_time = "21:00:00"
    end_time = "15:00:00"
    universe = "cu1803.SHF, cu1805.SHF"
    tk_to_h5(user, passwd,tkfile,universe,start_time,end_time,h5file)
    symbol = "cu1803.SHF"
